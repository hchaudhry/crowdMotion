package org.crowdMotion.utils;

public class Constants 
{

	
	public static String TOUR = "TOUR : ";
	public static String DEPLACEMENTS = "DEPLACEMENTS : ";
	public static String SOURIS_EN_DEPLACEMENT = "SOURIS EN DEPLACEMENT : ";
	public static String SOURIS_ARRIVEES = "SOURIS ARRIVEES : ";
	public static String PORTE_1 = "PORTE 1";
	public static String PORTE_2 = "PORTE 2";
	public static String VITESSE = "VITESSE";
	public static String LANCER = "LANCER";
	
	public static final int TILE_SIZE = 26;
	
	public static final char MUR = '*';
	public static final char ZONE_HERBE = 'G';
	public static final char ZONE_DEPLACEMENT = ' ';
	public static final char POINT_APPARITION = 'D';
	public static final char POINT_ARRIVEE = 'A';
	public static final char SOURIS = 'S';
	
	
}
